...
const InterviewHistory = () => {
  ...
  const [filter,     setFilter]     = useState("all"); // all | role | difficulty
  const [filterVal,  setFilterVal]  = useState("all");
  const [stats,      setStats]      = useState({ total: 0, avgScore: 0, bestRole: "—", lastDate: "—", bestScore: 0 });
        ...
        <div className="ih-charts-grid">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={lineData} margin={{ top: 10, right: 20, left: -10, bottom: 0 }}>
                ...
                <ReferenceLine y={stats.avgScore} stroke="rgba(0,229,255,0.3)" strokeDasharray="4 4"
                  label={{ value: "Avg", fill: "#00e5ff", fontSize: 10, position: "insideTopRight" }} />
          ...
          <div className="ih-chart-card">
            <div className="ih-chart-header">
              ...
              <span className="ih-chart-sub">{barData.length} role{barData.length !== 1 ? "s" : ""}</span>
            </div>
            <ResponsiveContainer width="100%" height={220}>
        ...
        {filtered.map(item => {
          ...
          return (
            <div key={item._id} className="ih-row">
              <span className="ih-role">
                <span className="ih-role-dot" />
              ...
              <span className="ih-score-cell">
                ...
                <div className="ih-score-bar-track">
                  <div className="ih-score-bar-fill" style={{ width: `${item.totalScore}%`, background: scoreColor }} />
                </div>
              </span>
              <span className="ih-verdict" style={{ color: vColor, borderColor: `${vColor}40`, background: `${vColor}10` }}>
                {item.verdict}
              </span>
